...
const AppointmentCard = ({ slot, view, onClick, isMobile }) => {
    ...
    // Mobile: full cell color, no padding/border-radius
    // Desktop: card style with padding and rounded corners
    const cardClass = isMobile
        ? `${getCardStyle()} h-full flex flex-col cursor-pointer`
    ...
    return (
        <div className={cardClass} onClick={onClick}>
            ...
            {(slot.badge || slot.icon) && (
                <div className={`flex items-center gap-0.5 ${isMobile ? "mb-0" : "gap-1 mb-1"}`}>
                    {slot.badge && (
                        <span className={`badge ${slot.badge === "G" ? "badge-work" : "badge-alert"} ${isMobile ? "text-[6px] w-3 h-3" : ""}`}>
                            {slot.badge}
                        </span>
                    )}
            ...
            {/* Name */}
            {slot.name && (
                <div className={`appointment-name whitespace-normal break-words ${isMobile ? "text-[7px] leading-[9px]" : "text-[9px] md:text-[11px] leading-3 md:leading-tight"}`}>{slot.name}</div>
            )}

            {/* Service - visible only in Day view */}
            {view === 'day' && slot.service && (
                <div className={`${isMobile ? "mt-0" : "mt-0.5 md:mt-auto pt-0.5 md:pt-1"}`}>
                    <span className={`appointment-service whitespace-normal break-words block ${isMobile ? "text-[6px] leading-[8px]" : "text-[8px] md:text-[10px] leading-3"}`}>{slot.service}</span>
                </div>
            )}
        </div>
    );
    ...
    return (
        ...
        >
            <div className={`flex-shrink-0 ${heightClass} flex items-center justify-center border-r border-b border-neutral-100`}>
                <span className={`font-medium text-neutral-500 ${isMobile ? "text-[8px] tracking-tighter" : "text-[10px] md:text-[13px]"}`}>
