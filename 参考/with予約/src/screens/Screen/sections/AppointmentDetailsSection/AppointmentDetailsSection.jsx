...
export const AppointmentDetailsSection = () => {
  ...
  return (
    ...
    >
      ...
      >
        {viewOptions.map((option, index) => (
          <button
            ...
            className={`absolute top-[3px] ${index === 0 ? "left-[calc(50.00%_-_70px)]" : "left-[71px]"} w-[73px] h-9 flex justify-center rounded-[20.49px] overflow-hidden ${
          ...
          >
            <span
              className={`flex items-center justify-center ml-[-0.2px] w-[13px] [font-family:'Noto_Sans_JP',Helvetica] font-normal ${
                selectedView === option.id ? "text-white" : "text-[#999999]"
              } text-[13px] text-center tracking-[0.91px] leading-[35.5px] whitespace-nowrap`}
            >
              {option.label}
            </span>
          </button>
      ...
      <button
        className="absolute top-[calc(50.00%_-_348px)] left-[calc(50.00%_-_1px)] w-[23px] h-[23px] rotate-[-90.00deg] aspect-[1]"
        aria-label="検索"
      >
        <img
      ...
      >
        <img
          className="absolute w-[66.67%] h-[66.67%] top-[13.84%] left-[13.83%]"
          alt=""
      ...
      >
        <button
          className="absolute top-[3px] left-[calc(50.00%_-_48px)] w-[85px] h-9 rounded-[20.49px] overflow-hidden"
          aria-label="店舗を選択"
